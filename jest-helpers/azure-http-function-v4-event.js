...
const azureHttpFunctionV4Event = {
...
function makeAzureHttpFunctionV4Event (values = {}) {
  process.env.FUNCTIONS_EXTENSION_VERSION = '~4'

  const baseEvent = clone(azureHttpFunctionV4Event)
  const mergedEvent = mergeDeep(baseEvent, values)

  mergedEvent.req.method = values.httpMethod
  mergedEvent.req.originalUrl = 'http://localhost:7071'
  mergedEvent.req.originalUrl += values.path
  ...
  if (values.multiValueQueryStringParameters) {
    ...
    mergedEvent.req.originalUrl += '?' + rawQueryString.toString()
  }
...
function makeAzureHttpFunctionV4Response (values = {}, { shouldConvertContentLengthToInt = false } = {}) {
  const baseResponse = {
    body: '',
...
module.exports = {
  makeAzureHttpFunctionV4Event,
  makeAzureHttpFunctionV4Response
}
